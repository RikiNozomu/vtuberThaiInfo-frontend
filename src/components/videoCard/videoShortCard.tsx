...
export default function VideoShortCard(props: {
    ...
    <div>
      ...
      <AspectRatio ratio={9 / 16}>
        <Paper
          ...
          shadow="sm"
          radius="md"
          className="h-full relative group overflow-clip hover:cursor-pointer as"
        >
          {props.isPin && (
          ...
          {(data.talents?.length as number) < 1 && (
            <div
              onClick={() => setOpened(true)}
              className={`flex text-left w-full gap-2 px-2 left-0 text-lg ${isMobile ? "bottom-2" : "group-hover:opacity-100 opacity-0 group-hover:bottom-2 bottom-0 transition-all ease-out duration-100"} absolute text-white`}
            >
              {data.title}
            </div>
          )}
